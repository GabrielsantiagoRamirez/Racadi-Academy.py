...
const buscarObservaciones = async () => {
  try {
    ...
    const response = await axios.get(`http://127.0.0.1:8889/filtro_ObservadoresDocumento/${usuario.value.documento}` ) ;
...
const buscarObservacionesFeceha = async () => {
  try {
    // Realiza la solicitud GET al endpoint de FastAPI
    const response = await axios.get(`http://127.0.0.1:8889/filtro_ObservadoresFecha/${usuario.value.documento}/${fecha.value}`);
