<template>
    ...
    <div class="container_principal_solicitudes">
        ...
        <div class="transparent-container">
            <div class="contenedor_tabla_solicitud">
                <table class="tabla_solicitud">
                    <thead class="contenedor_informacion_solicitud_header">
                        <td class="cabeza_tabla_solicitudes" colspan="5">Solicitudes no contestadas</td>
                        <tr>
                    ...
                    <tbody class="fondo_solicitudes" v-if="solicitud.length > 0">
                        <tr v-for="s in solicitud" :key="s.id_solicitud">
                            ...
                            <td class="informacion_solicitud">
                                <span>No te han enviado respuesta aún</span>
                            </td>
            ...
            <table class="tabla_solicitud_contestada">
                    <thead class="contenedor_informacion_solicitud_header">
                        <td class="cabeza_tabla_solicitudes" colspan="5">Solicitudes contestadas</td>
                        <tr>
                            <td class="cabeza_tabla_solicitudes">Id</td>
                            <td class="cabeza_tabla_solicitudes">Documento</td>
                            <td class="cabeza_tabla_solicitudes">Descripcion</td>
                    ...
                    <tbody class="fondo_solicitudes" v-if="solicitud_contestada.length > 0">
                        <tr v-for="s in solicitud_contestada" :key="s.id_solicitud">
                            <td class="informacion_solicitud">{{ s.id_solicitud }}</td>
                            <td class="informacion_solicitud">{{ s.documento }}</td>
                            <td class="informacion_solicitud">{{ s.descripcion }}</td>
                            <td class="informacion_solicitud">
...
const solicitud = ref([]);
const solicitud_documento= ref(null)
const solicitud_contestada = ref([]);
...
const info_solicitudes = async () => {
    ...
    try {
        ...
        console.log("Me encuentro aquí en traer datos de la solicitud");

        if (solicitud.value.length === 0) {
            console.log("No hay solicitudes no contestadas"); // Mensaje si no hay solicitudes
        }
...
// Función para obtener la información de las solicitudes contestadas
const info_solicitudes_contestadas = async () => {
    const token = localStorage.getItem('token');
    if (!token) {
        router.push('/login');
        return;
    ...
    try {
        const response = await axios.get(`http://127.0.0.1:8000/obtenersolicitudestudiante/${solicitud_documento.value.documento} `, 
        {
            headers: {
                Authorization: `Bearer ${token}`
            }
        ...
        // Asigna la respuesta a la variable de solicitud contestada
        solicitud_contestada.value = response.data;

        if (solicitud_contestada.value.length > 0) {
            console.log(solicitud_contestada.value.documento);
            console.log("Me encuentro aquí mostrando las solicitudes contestadas");
        } else {
            console.log("No hay solicitudes contestadas"); // Mensaje si no hay solicitudes
        }

    } catch (error) {
        console.error('Error fetching solicitudes:', error);
        // Manejo de errores específico si es necesario, pero no redirigir
    }
};
...
const fetchUserProfile = async () => {
        ...
        try {
          ...
          solicitud_documento.value = response.data; // Asigna los datos del usuario
          console.log(solicitud_documento.value.documento)
          console.log("Me encuentro aquí en donde pido el documento");
        } catch (error) {
...
// Montar la función cuando el componente está listo
onMounted(async () => {
    await fetchUserProfile(); // Primero obtener el perfil del usuario
    await info_solicitudes(); // Luego obtener las solicitudes
    await info_solicitudes_contestadas(); // Finalmente obtener las solicitudes contestadas
});
...
.tabla_solicitud_contestada {margin-top: 20px;
