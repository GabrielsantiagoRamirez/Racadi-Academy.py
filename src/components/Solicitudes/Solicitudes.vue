<template>
  ...
  const agregarSolicitud = async() =>{
    try{
        const response = await axios.post("http://127.0.0.1:8889/agregar_solicitud",{
  ...
  try {
    await axios.delete(`http://127.0.0.1:8889/eliminar_solicitud/${id_solicitud}`);
    Swal.fire({
      icon: 'success',
  ...
  const obtener_solicitudes = async () => {
    try {
      const response = await axios.get(`http://127.0.0.1:8889/obtenersolicitudestudiante/${usuario.value.documento}`);
