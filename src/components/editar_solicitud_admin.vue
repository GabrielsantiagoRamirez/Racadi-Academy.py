<template>
    <div class="contenedor_solicitud_form">
        ...
        <form @submit.prevent="contestar_solicitud" class="formulario_solicitud">
            ...
            <div class="formulario_grupo_editar">
                <label for="documento">
                    <i class="fa fa-id-card"></i> Documento a enviar
                </label>
            ...
            <div class="formulario_grupo_editar">
                ...
                <input type="text" v-model="respuesta" id="respuesta" required>
            </div>

            <button type="submit" class="enviar_solicitud">
                <i class="fa fa-save"></i> Guardar Solicitud Contestada
            </button>
        </form>
    </div>
</template>
...
const contestar_solicitud = async () => {
    try {
        const response = await axios.put(`http://127.0.0.1:8000/actualizar_contestacion/${id.value}/${documento.value}/${respuesta.value}`, {
        ...
        Swal.fire({
            icon: 'success',
            title: 'Contestando Solicitudes',
            text: 'Contestación actualizada con éxito'
        });
